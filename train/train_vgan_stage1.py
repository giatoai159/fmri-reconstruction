...
if __name__ == "__main__":
    ...
    parser.add_argument('--pretrained_gan', '-pretrain',
                        default=gan_cfg.pretrained_gan, help='pretrained gan', type=str)
    ...
    parser.add_argument('--margin', default=gan_cfg.margin,
                        type=float, help='margin for generator/discriminator game')
    ...
    parser.add_argument('--beta', default=gan_cfg.beta,
                        type=float, help='beta factor for beta-vee')
    parser.add_argument('--mode', default='vae-gan',
                        help='vae, vae-gan, beta-vae, dcgan')
    ...
    file_handler = logging.FileHandler(
        os.path.join(args.logs, 'train_gan_' + timestep))
    logger = logging.getLogger()
    ...
    if DEBUG:
        saving_dir = os.path.join(
            SAVE_PATH, 'debug', 'debug_gan_{}'.format(timestep))
    else:
    ...
    if args.pretrained_gan is not None:
        pretrained_model_dir = os.path.join(
            SAVE_PATH, 'gan', args.pretrained_gan, args.pretrained_gan + '.pth')
    ...
    if args.dataset == 'coco':
        ...
        dataloader_train = DataLoader(
            train_test_data, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    ...
    model = VaeGan(device=device, z_size=args.latent_dim,
        ...
        try:
            ...
            for batch_idx, data_batch in enumerate(dataloader_train):
                ...
                if args.dataset == 'coco':
                    ...
                    x = Variable(
                        data_batch, requires_grad=False).float().to(device)

                elif args.dataset == 'mnist':
                    batch_size = len(data_batch[0])
                    x = Variable(
                        data_batch[0], requires_grad=False).float().to(device)
                else:
                    logging.ingo('Wrong dataset')
            ...
            if not idx_epoch % 2:
                ...
                ax.set_xticks([])
                ax.set_yticks([])
                ax.imshow(make_grid(x[: 25].cpu().detach(),
            ...
            for batch_idx, data_batch in enumerate(dataloader_valid):
                ...
                if idx_epoch == 0:
                    ...
                    ax.set_xticks([])
                    ax.set_yticks([])
                    ax.imshow(make_grid(data_in[: 25].cpu().detach(
                    ), nrow=5, normalize=True).permute(1, 2, 0))
                    gt_dir = os.path.join(
                        images_dir, 'epoch_' + str(idx_epoch) + '_ground_truth_' + 'grid')
                    plt.savefig(gt_dir)
                ...
                output_dir = os.path.join(
                    images_dir, 'random', 'epoch_' + str(idx_epoch) + '_output_' + 'rand')
                plt.savefig(output_dir)
