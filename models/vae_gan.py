    ...
    def __init__(self, channel_in=3, z_size=128):
        ...
        for i in range(3):
            layers_list.append(EncoderBlock(
                channel_in=self.size, channel_out=config.encoder_channels[i]))
        ...
        self.fc = nn.Sequential(nn.Linear(in_features=config.fc_input * config.fc_input * self.size,
                                ...
                                nn.BatchNorm1d(
                                    num_features=config.fc_output, momentum=0.9),
                                nn.ReLU(True))
        # two linear to get the mu vector and the diagonal of the log_variance
        self.l_mu = nn.Linear(
            in_features=config.fc_output, out_features=z_size)
...
class Decoder(nn.Module):
    ...
    def __init__(self, z_size, size):
        ...
        self.fc = nn.Sequential(nn.Linear(in_features=z_size, out_features=config.fc_input * config.fc_input * size, bias=False),
                                nn.BatchNorm1d(
                                    num_features=config.fc_input * config.fc_input * size, momentum=0.9),
                                nn.ReLU(True))
        self.size = size
        layers_list = []
        layers_list.append(DecoderBlock(channel_in=self.size,
                           channel_out=self.size, out=config.output_pad_dec[0]))
        layers_list.append(DecoderBlock(
            channel_in=self.size, channel_out=config.decoder_channels[1], out=config.output_pad_dec[1]))
        self.size = config.decoder_channels[1]
        layers_list.append(DecoderBlock(
            channel_in=self.size, channel_out=config.decoder_channels[2], out=config.output_pad_dec[2]))
        self.size = config.decoder_channels[2]
        # final conv to get 3 channels and tanh layer
        layers_list.append(nn.Sequential(
            nn.Conv2d(in_channels=self.size,
                      out_channels=config.decoder_channels[3], kernel_size=5, stride=1, padding=2),
            nn.Tanh()
        ))
...
class Discriminator(nn.Module):
    ...
    def __init__(self, channel_in=3, recon_level=3):
        ...
        self.conv.append(nn.Sequential(
            nn.Conv2d(
                in_channels=3, out_channels=config.discrim_channels[0], kernel_size=5, stride=config.stride_gan, padding=2),
            nn.ReLU(inplace=True)))
        self.size = config.discrim_channels[0]
        self.conv.append(EncoderBlock(channel_in=self.size,
                         channel_out=config.discrim_channels[1]))
        self.size = config.discrim_channels[1]
        self.conv.append(EncoderBlock(channel_in=self.size,
                         channel_out=config.discrim_channels[2]))
        self.size = config.discrim_channels[2]
        self.conv.append(EncoderBlock(channel_in=self.size,
                         channel_out=config.discrim_channels[3]))
...
class VaeGan(nn.Module):
    ...
    def __init__(self, device, z_size=128, recon_level=3):
        ...
        self.decoder = Decoder(
            z_size=self.z_size, size=self.encoder.size).to(device)
        self.discriminator = Discriminator(
            channel_in=3, recon_level=recon_level).to(device)
    ...
    def forward(self, x, gen_size=10):
        ...
        if self.training:
            ...
            if x is None:
                z_p = Variable(torch.randn(gen_size, self.z_size).to(
                    self.device), requires_grad=False)  # just sample and decode
                x_p = self.decoder(z_p)
                return x_p
            else:
                mus, log_variances = self.encoder(x)
    ...
    def loss(x, x_tilde, disc_layer_original, disc_layer_predicted, disc_layer_sampled, disc_class_original,
        ...
        mse = torch.sum(0.5 * (disc_layer_original -
                        disc_layer_predicted) ** 2, 1)
...
class VaeGanCognitive(nn.Module):
    ...
    def forward(self, sample, gen_size=10):
        ...
        if sample is not None:
            ...
            if self.training:
                ...
                # discriminator for reconstruction
                disc_layer = self.discriminator(gt_x, x_tilde, x_p, "REC")
    ...
    def loss(gt_x, x_tilde, disc_layer_original, disc_layer_predicted, disc_layer_sampled, disc_class_original,
        ...
        # reconstruction error, not used for the loss but useful to evaluate quality
        nle = 0.5 * (gt_x.view(len(gt_x), -1) -
                     x_tilde.view(len(x_tilde), -1)) ** 2
        # mse_loss = nn.MSELoss()
        # nle = mse_loss(gt_x, x_tilde)

        # kl-divergence
        kld_weight = 1
        kld = -0.5 * torch.sum(-variances.exp() -
                               torch.pow(mus, 2) + variances + 1, 1) * kld_weight
