...
class Bold5000Parser(object):
    ...
    def coco_mapping(self):
        ...
        for (dirpath, dirs, files) in os.walk(os.path.join(self.root_dir, self.bold_stimuli_dir, 'COCO')):
            for filename in files:
                ...
                dataframe['identifier'].append(
                    get_identifier(filename, 'coco'))
        ...
        bold_coco_annotations = pd.DataFrame(dataframe, index=pd.Index(
            dataframe['identifier'], name='identifier'))
    ...
    def imagenet_mapping(self):
        ...
        for (dirpath, dirs, files) in os.walk(os.path.join(self.root_dir, self.bold_stimuli_dir, 'ImageNet')):
            for filename in files:
                dataframe['filename'].append(filename)
                dataframe['identifier'].append(
                    get_identifier(filename, 'imagenet'))
                dataframe['synset'].append(get_identifier(
                    filename, 'imagenet').split('_')[0])
        ...
        annotation_list = Group(Word(
            alphanums + ' -\'.') + ZeroOrMore(Suppress(',') + Word(alphanums + ' -\'.')))
        annotation_pattern = OneOrMore(
            identifier + annotation_list + ZeroOrMore(Suppress(LineEnd())))
        parsed = annotation_pattern.parseString(annotations).asList()

        # unravel list of lists to clean dataframe
        imagenet_annotations = pd.DataFrame(
            {'synset': parsed[::2], 'raw_annotations': parsed[1::2]})
        ...
        bold_imagenet_annotations = pd.DataFrame(
            dataframe, index=pd.Index(dataframe['identifier'], name='identifier'))
    ...
    def scene_mapping(self):
        ...
        for (dirpath, dirs, files) in os.walk(os.path.join(self.root_dir, self.bold_stimuli_dir, 'Scene')):
            for filename in files:
                dataframe['filename'].append(filename)
                dataframe['identifier'].append(
                    get_identifier(filename, 'scene'))
                dataframe['source'].append('scene')
                dataframe['scene_label'].append(
                    label_finder.match(filename).group())
        ...
        bold_scene_annotations = pd.DataFrame(
            dataframe, index=pd.Index(dataframe['identifier'], name='identifier'))
    ...
    def session_data(self):
        ...
        event_file_pattern = re.compile(
            r'sub-CSI\d_ses-\d\d_task-5000scenes_run-\d\d_events\.tsv')
        ...
        bold_session_data = pd.concat(
            [pd.read_csv(file, sep='\t') for file in session_file_list])
        bold_session_data.columns = [snake_case(
            c) for c in bold_session_data.columns]
...
if __name__ == "__main__":
    ...
    COCO_ANNOTATION_FILE = os.path.join(
        DATA_ROOT, data_cfg.coco_annotation_file)
    SAVE_PATH = os.path.join(
        args.output, data_cfg.data_root, data_cfg.save_path)
