...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', '-b', default=inf_cfg.batch_size,
    ...
    parser.add_argument('--image_size', '-im_size', default=inf_cfg.image_size, help='size to which image should '
                        'be scaled', type=int)
    parser.add_argument('--device', '-d', default=inf_cfg.device,
    ...
    parser.add_argument('--dataset', default=inf_cfg.dataset,
                        help='dataset type, "coco", "bold" ', type=str)
    parser.add_argument('--mode', default=inf_cfg.mode,
    ...
    file_handler = logging.FileHandler(
        os.path.join(args.logs, 'inference' + timestep))
    logger = logging.getLogger()
    ...
    if DEBUG:
        saving_dir = os.path.join(
            SAVE_PATH, 'debug', 'inference_{}_{}'.format(args.pretrained_gan, timestep))
    else:
        if inf_cfg.save_to_folder is not None:
            ...
            saving_dir = os.path.join(SAVE_PATH, 'inference', 'inference_{}_{}'.format(
                args.pretrained_gan, timestep))
    if not os.path.exists(saving_dir):
        os.makedirs(saving_dir)
    if args.pretrained_gan is not None:
        pretrained_model_dir = os.path.join(
            SAVE_PATH, inf_cfg.folder_name, args.pretrained_gan, args.pretrained_gan + '.pth')
    saving_name = os.path.join(
        saving_dir, 'inference.pth'.format(args.pretrained_gan))

    if inf_cfg.file_to_save is not None:
        saving_file = os.path.join(
            SAVE_PATH, 'inference', inf_cfg.file_to_save)
    ...
    if args.dataset == 'coco':
        ...
        if args.mode == 'vae-gan':
            ...
            decoder = Decoder(z_size=args.latent_dim,
                              size=encoder.size).to(device)
            teacher_model = VaeGan(
                device=device, z_size=args.latent_dim).to(device)
    ...
    if args.pretrained_gan is not None and os.path.exists(pretrained_model_dir.replace(".pth", ".csv")):
        ...
        if inf_cfg.evaluate:
            ...
            for top in [2, 5, 10]:
                obj_pcc, obj_ssim = objective_assessment(
                    model, dataloader_valid, dataset=args.dataset, mode=args.mode, top=top)
            ...
            y_axis = [obj_score['pcc'][0],
                      obj_score['pcc'][1], obj_score['pcc'][2]]
            ...
            for i, bar in enumerate(bars):
                yval = bar.get_height()
                plt.text(bar.get_x() + 0.10, yval + .005,
                         f'{y_axis[i].item() * 100:.2f}')
    ...
    structural_similarity = StructuralSimilarity(
        mean=inf_cfg.mean, std=inf_cfg.std)
    mse_loss = nn.MSELoss()
    ...
    for batch_idx, data_batch in enumerate(dataloader_valid):
        ...
        with no_grad():
            ...
            if inf_cfg.save:
                ...
                gt_dir = os.path.join(
                    images_dir, 'batch_' + str(batch_idx) + '_ground_truth_' + 'grid')
                plt.savefig(gt_dir)
                ...
                output_dir = os.path.join(
                    images_dir, 'batch_' + str(batch_idx) + '_output_' + 'grid')
